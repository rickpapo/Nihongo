...
#define NaAdjDaBasicRentaiForm   (* * ナ形容詞 ダ列基本連体形) \
			...
			(* * ナノ形容詞 ダ列特殊連体形)
		;; ナ形容詞の連体形 (判定詞は除く)
#define NaAdjDaTaBasicForm (* * ナ形容詞 ダ列タ形) \
			(* * ナ形容詞特殊 ダ列タ形) \
			(* * ナノ形容詞 ダ列タ形) \
			(* * 判定詞 ダ列タ形)
#define NaAdjDearuBasicBasicForm   (* * ナ形容詞 デアル列基本形) \
			(* * ナ形容詞特殊 デアル列基本形) \
			(* * ナノ形容詞 デアル列基本形) \
			(* * 判定詞 デアル列基本形)
#define NaAdjDearuTaBasicForm   (* * ナ形容詞 デアル列タ形) \
			(* * ナ形容詞特殊 デアル列タ形) \
			(* * ナノ形容詞 デアル列タ形) \
			(* * 判定詞 デアル列タ形)
#define NaAdjDesuBasicBasicForm   (* * ナ形容詞 デス列基本形) \
			(* * ナ形容詞特殊 デス列基本形) \
			(* * ナノ形容詞 デス列基本形) \
			(* * 判定詞 デス列基本形)
#define NaAdjDesuTaBasicForm   (* * ナ形容詞 デス列タ形) \
			(* * ナ形容詞特殊 デス列タ形) \
			(* * ナノ形容詞 デス列タ形) \
			(* * 判定詞 デス列タ形)
#define NaAdjBasicForm  NaAdjDaTaBasicForm \
			NaAdjDearuBasicBasicForm \
			NaAdjDearuTaBasicForm \
...
#define NaAdjDaTaRenyoTeForm (* * ナ形容詞 ダ列タ系連用テ形) \
			(* * ナ形容詞特殊 ダ列タ系連用テ形) \
			(* * ナノ形容詞 ダ列タ系連用テ形) \
			(* * 判定詞 ダ列タ系連用テ形)

#define NaAdjDearuBasicRenyoForm (* * ナ形容詞 デアル列基本連用形) \
			(* * ナ形容詞特殊 デアル列基本連用形) \
			(* * ナノ形容詞 デアル列基本連用形) \
			(* * 判定詞 デアル列基本連用形)
...
; 括弧，空白は透過処理をするが，括弧だけ，空白だけなどの特別の場合を処
; 理するために文頭，文末と連接可能としておく．
...
; 括弧始の後に接続詞を許すために,括弧始の前の普通の接続を与える
...
(((動詞 * * 基本連用形)
  (接尾辞 動詞性接尾辞 * 基本連用形)
  NaAdjDearuBasicRenyoForm)			; 「～でありうる」
        ((接尾辞 動詞性接尾辞 動詞性接尾辞うる型 * うる)
         (接尾辞 動詞性接尾辞 動詞性接尾辞得る型 * 得る)))
...
(((動詞 * * 基本連用形)
  ...
  NaAdjDearuBasicRenyoForm)		; 「～であります」
...
(((動詞 * 母音動詞 未然形)
  ...
  ; 動詞性接尾辞ます型 とはつながらない
  (接尾辞 動詞性接尾辞 動詞性接尾辞うる型 未然形)
  (接尾辞 動詞性接尾辞 動詞性接尾辞得る型 未然形)
  IAdjBasicRenyoForm
  NaAdjDaTaRenyoTeForm
...
; ※ ただし「東京都」「京都府」「大阪府」は連語,「北海道」は一語で扱っている
...
(((形容詞 * * 語幹)
  ...
  (接尾辞 形容詞性名詞接尾辞 * 語幹))
        ((接尾辞 名詞性述語接尾辞 * * さ)
         (接尾辞 名詞性述語接尾辞 * * め)
         (接尾辞 名詞性述語接尾辞 * * 気)       ; kuro 93/10/19
         (接尾辞 名詞性述語接尾辞 * * げ))      ; kuro 93/10/19
)
...
((NaAdjDearuBasicRenyoForm)		; 「～でありさえ」
        ((助詞 副助詞 * * さえ)
         (助詞 副助詞 * * こそ)))
...
; ※ 用言と「に」の接続はかなり制限されるはずだが，とりあえずBasicFormは
; すべて接続するとしておく．現在わかっていることは,
;
;	○ 動詞基本連用形は自由に節をつくる(「思うに」)
;	○ その他は基本的に後の用言が限られるはず
;		「にふさわしい」
;		「にちがいない」
;		「に限る」
...
((VerbBasicBasicForm
  ...
  NaAdjDaTaBasicForm         	; ?「学生だったまではよかった」
  NaAdjDearuTaBasicForm)        ; ?「学生であったまではよかった」
        ((助詞 接続助詞 * * まで)))
...
(((動詞 * * 基本連用形)
  (接尾辞 動詞性接尾辞 * 基本連用形)
  NaAdjStem				;「静かながら」
  NaAdjDearuBasicRenyoForm		;「静かでありながら」
...
((VerbBasicForm			; 「書くだろう」
  ...
  NaAdjDaTaBasicForm		; 「～だっただろう」
  NaAdjDearuBasicBasicForm	; 「～であるだろう」
  NaAdjDearuTaBasicForm		; 「～であっただろう」
...
((VerbBasicForm
  ...
  NaAdjDaTaBasicForm		; 「静かだったそうだ」
  NaAdjDearuBasicBasicForm	; 「静かであるそうだ」
  NaAdjDearuTaBasicForm		; 「静かであったそうだ」
				; ※デス列は付かない
  (助動詞 * 助動詞ぬ型 基本形)
  (助動詞 * 助動詞ぬ型 音便基本形)
  (助動詞 * 助動詞ぬ型 タ形))
...
((VerbBasicForm         	;「書くではない(か)」
  ...
  NaAdjDaTaBasicForm		;「静かだったではない(か)」
  NaAdjDearuBasicBasicForm	;「静かであるではない(か)」
  NaAdjDearuTaBasicForm)        ;「静かであったではない(か)」
        ((連語 * イ形容詞アウオ段 * ではない)
         (連語 * イ形容詞アウオ段 * じゃない)
         (連語 * イ形容詞アウオ段 * でもない)))
