...
static void hiragana_encode(FILE *fp, unsigned char *str)
{
	...
	while (*str) {
		//get ucs2 code val
		if (*str < 0xC0 || *str > 0xEF || (*str < 0xE0 && ((0xE0 & *str) != 0xC0 || (0x80 & *(str + 1)) != 0x80)) ||
			(*str >= 0xE0 && ((0xF0 & *str) != 0xE0 || (0x80 & *(str + 1)) != 0x80 || (0x80 & *(str + 2)) != 0x80))) {
			error(OtherError, "An invalid char was encountered in hiragana_encode(). This function only handles 2- and 3-byte "
				"UTF-8 characters, e.g. no ascii.", EOA);
		}
		if (*str < 0xE0) {
			code = ((*(str) & 0x1F) << 6) +
				(*(str+1) & 0x3F);
			fputc(*(str), fp);
			fputc(*(str+1), fp);
			str += 2;	//length of hiragana character in this range of UTF-8.
		} else {
			code = ((*(str) & 0x0F) << 12) +
				((*(str+1) & 0x3F) << 6) +
				(*(str+2) & 0x3F);
			if (code >= 0x3041 && code < 0x3097) {
				fputc(code - 0x3040 + 0x20, fp);	//0x3040 is 1 beneath the first hiragana character. Re-increment by 0x20 to get above the control chars.
			} else {
				fputc(*(str), fp);
				fputc(*(str+1), fp);
				fputc(*(str+2), fp);
			}
			str += 3;	//length of hiragana character in this range of UTF-8.
		}
	}
    fputc(0x20, fp);
}
