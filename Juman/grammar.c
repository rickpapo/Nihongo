...
void read_class(FILE *fp)
{
     ...
     while (! s_feof(fp)) {
	  ...
	  if ( !Null(cell2 = car(cell1)) ) {
	       Class[i][j].id = (U_CHAR *)
		 my_alloc((int)((sizeof(U_CHAR)*strlen(_Atom(car(cell2)))) + 1));
...
void grammar ( FILE *fp_out ) {
	...
	while ( 1 ) {
		if ( ( fp = pathfopen ( GRAMMARFILE, "r", "", grammarfile_path ) ) != NULL )
			break ;
		if ( ( fp = pathfopen ( GRAMMARFILE, "r", CurPath, grammarfile_path ) ) != NULL ) 
			break ;
		if ( ( fp = pathfopen ( GRAMMARFILE, "r", JumanPath, grammarfile_path ) ) != NULL ) 
			break ;
		error ( (int*(*)(void))OpenError, "can't open", GRAMMARFILE, ".", EOA ) ;
	}
