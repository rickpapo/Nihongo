...
BOOL juman_init_rc ( FILE *fp ) {
	...
	*/
	...
	/* 文法ファイル */
	num = 0 ;
	GetPrivateProfileString ( "juman", "dicfile", ".\\Juman-5.1\\dic", Jumangram_Dirname, sizeof(Jumangram_Dirname), "juman.ini" ) ;
	if ( Jumangram_Dirname[0] ) {
		...
		connect_matrix ( NULL ) ;
		/* 辞書ファイル */
		GetPrivateProfileString ( "juman", "dicfile", ".\\Juman-5.1\\dic", dic_file_name, sizeof(dic_file_name), "juman.ini" ) ;
		...
		/* 辞書ファイル */
			...
			for(num = 0 ; ; num++) {
				if( Null(car(cell2)) )  break;
					...
					error ( (int*(*)(void))ConfigError, "Too many dictionary files.", EOA);
				else {
...
BOOL juman_close ( void ) {
  ...
  for (i = 0; i < TBL_NUM; i++) {
	  RENSETU_PAIR *pPair = &rensetu_tbl[i];
	  if ( pPair->goi )
		free(pPair->goi);
  }
...
------------------------------------------------------------------------------
*/
extern int get_hinsi_id ( unsigned char *x ) ;
extern int get_bunrui_id ( unsigned char *x, int hinsi ) ;
...
void read_class_cost ( CELL *cell ) {
	...
	while ( !Null ( car ( cell ) ) ) {
		...
		if ( !strcmp ( _Atom ( car ( pos_cell ) ), "*" ) ) {
			...
			if ( Null ( car ( cdr ( pos_cell ) ) ) || !strcmp ( _Atom ( car ( cdr ( pos_cell ) ) ), "*" ) ) {
				/* 細分類が * または無しなら品詞全体に */
				for (bunrui = 0; Class[hinsi][bunrui].id ; bunrui++)
					Class[hinsi][bunrui].cost = cost;
			}
...
static BOOL katuyou_process ( int position, int *k, MRPH mrph, int *length ) {
	while ( Form[mrph.katuyou1][*k].name ) {
		if ( compare_top_str1 ( Form[mrph.katuyou1][*k].gobi, String + position + mrph.length ) ) {
			*length = mrph.length + (int) strlen ( Form[mrph.katuyou1][*k].gobi ) ;
			return ( TRUE ) ;
		} else {
			(*k)++;
		}
...
#define 	RENGO_BUFSIZE		20
	...
	for ( i=0; i<num; i++ ) { /* まず，連語全体をバッファに読み込む */
		...
		length = (int) strlen ( new_mrph->midasi ) ;
		if ( Class[new_mrph->hinsi][new_mrph->bunrui].kt ) /* 活用する */
			if (i < num-1) { /* 末尾以外の活用語 */
				length += (int) strlen ( Form[new_mrph->katuyou1][new_mrph->katuyou2].gobi ) ;
				new_mrph->con_tbl += ( new_mrph->katuyou2 - 1 ) ;
			}
			new_mrph->length = length ;

			if ( i < num-1 ) 
				pos += length ; /* 末尾形態素の位置を保存 */
...
/*printf("  TO %d\n", new_mrph->con_tbl);*/
			...
			if (add_list[co]) {
			    ...
			    glen = (int) strlen ( Form[new_mrph->katuyou1] [new_mrph->katuyou2].gobi ) ;
...
char *_take_data(char *s , MRPH *mrph)
{
    ...
    mrph->length   = (int) strlen(mrph->midasi);
...
void hiragana_decode(char **str, char *yomi)
{
	...
	if (*s < 0x80) {
	    *(yomi++) = 0xE3;
		...
		if ( *s < (char)0xE0 ) {	//two-byte UTF-8 char
			*(yomi++) = *(s++);
			*(yomi++) = *(s++);
...
int undef_word ( int pos ) {
	...
	if ( ( unicode_val != 0x20 ) /* ascii space */ && ( char_ublock != HIRAGANA_BLOCK ) && ( char_ublock != KANJI_BLOCK ) ) {
		while ( 1 ) {
			...
			if ( ( next_unicode_val > 0 ) 
				&& ( ( char_ublock == next_char_ublock ) 
...
int 	suusi_word(int pos , int m_num)
{
	...
	if (pre_mrph->hinsi == suusi_hinsi &&
	    ...
	    m_buffer[m_buffer_num].length += (int) strlen(new_mrph->midasi);
	    /* コストは後ろ側の方を継承 */
	    ...
	    p_buffer[p_buffer_num].end = pos+(int)strlen(new_mrph->midasi);
	    p_buffer[p_buffer_num].mrph_p = m_buffer_num;
...
MRPH *prepare_path_mrph ( int path_num , int para_flag ) {
	...
	if ( ( para_flag != 0 ) && ( is_through(mrph_p) == TRUE ) ) 
		return ( NULL ) ;
...
char *get_path_mrph(int path_num , int para_flag)
{
    ...
    len = (int) ( strlen(kigou)+strlen(midasi1)+strlen(yomi)+strlen(midasi2)+strlen(Class[mrph_p->hinsi][0].id)+mrph_p->hinsi/10+1 ) ;

    if ( mrph_p->bunrui ) {
	len += (int)strlen(Class[mrph_p->hinsi][mrph_p->bunrui].id);
    }
    else {
	len += 1;
    }
    ...
    if ( mrph_p->katuyou1 ) {
	len += (int) strlen(Type[mrph_p->katuyou1].name);
    }
    else {
	len += 1;
    }
    ...
    if ( mrph_p->katuyou2 ) {
	len += (int) strlen(Form[mrph_p->katuyou1][mrph_p->katuyou2].name);
    }
    else {
	len += 1;
    }
...
void print_path_mrph(FILE* output, int path_num , int para_flag)
{
    ...
    switch (Show_Opt2) {
    case Op_F:
	len = (int) strlen(yomi); yomi[len] = ')'; yomi[len+1] = '\0';
	...
	if (!strcmp(midasi1, "\\ ")) pos++; else pos += (int)strlen(midasi1);
	fprintf(output, "%d ", pos);
	/* -E オプションはこの部分だけ追加,
