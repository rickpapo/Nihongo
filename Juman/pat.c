...
extern long bmp_code_point ( unsigned char *cstr ) ;
...
pat_node *pat_search(FILE *f, char *key, pat_node *x_ptr, char *rslt)
{
  ...
  int key_length = (int) strlen(key); /* キーの文字数を数えておく */
...
/*  rslt[0] = '\0';*/
  ...
  do {
    ptr = x_ptr;
	unicode_val = bmp_code_point(key);	//-1 indicates a string invalid for UTF-8
    /* 敷居ビットならば */
    OL(checkbit:)OI(ptr->checkbit);
    if(ptr->checkbit % 8 == 0 && ptr->checkbit != 0 && unicode_val > 0){ /* 途中単語を探す */
      tmp_x_ptr = ptr;
      do { /* 左部分木の一番左のノードを調べる． */
	tmp_ptr = tmp_x_ptr;
...
{
  pat_node *ptr,*top_ptr = x_ptr,*tmp_x_ptr = x_ptr;
  pat_index_list *tmp_l_ptr;
  int in_hash;
  int key_length = (int) strlen(key); /* キーの文字数を数えておく */
  char buffer[50000]; /* 汎用バッファ */
...
void pat_insert(FILE *f,char *line, long index, pat_node *x_ptr, char *kugiri)
{
  ...
  key_length = (int) strlen(key); /* キーの文字数を数えておく */
...
void show_pat(pat_node *top_ptr, FILE *out_to, char *prefix)
{
  ...
  /* 敷居ビットのとき */
  unicode_val = bmp_code_point(key);	//-1 indicates a string invalid for UTF-8
  if(top_ptr->checkbit % 8 == 0 && top_ptr->checkbit != 0 && unicode_val > 0){
