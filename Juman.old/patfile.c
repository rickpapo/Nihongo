...
void com_l ( char *fname_pat, pat_node *ptr ) {
	if ( ( fd_pat = _open ( fname_pat, O_RDONLY|O_BINARY ) ) == -1 ) {
...
unsigned char egetc(int file_discripter){
	...
	if(++ctr == sizeof(buf)){
		ctr = 0;
		_read ( file_discripter, buf, sizeof(buf) ) ;
		/* OL(.);fflush(stdout);*/
	}
...
void com_s ( char *fname_pat, pat_node *ptr ) {
	...
	if ( ( fd_pat = _open ( fname_pat, O_WRONLY|O_CREAT|O_BINARY, 0644 ) ) == -1 ) {
		fprintf ( stderr, "ERROR: Unable to open \"%s\".\n", fname_pat ) ;
		exit ( 1 ) ;
	};
...
void save_pat ( pat_node *top_ptr ) {
	...
	if ( top_ptr->checkbit < top_ptr->left->checkbit )
		...
		if ( ptr->index < 0 ) 
			...
			while ( ptr != NULL ) {
				if ( ptr->next == NULL ) 
					eputc ( ( ( ptr->index >> 24 ) & 0x3f ) | 0xc0, fd_pat ) ;
				else 
					eputc ( ( ( ptr->index >> 24 ) & 0x3f ) | 0x80, fd_pat ) ;
...
void eputc ( unsigned char c, int file_discripter ) {
	...
	if ( ctr == BUFSIZ ) {
		ctr = 0 ;
		_write ( file_discripter, buf, BUFSIZ ) ;
	}
}
