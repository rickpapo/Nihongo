...
void connect_table ( FILE *fp_out ) {
	...
	while ( 1 ) {
		if ( ( fp = pathfopen ( TABLEFILE, "r", "", tablefile_path ) ) != NULL )
			break ;
		if ( ( fp = pathfopen ( TABLEFILE, "r", CurPath, tablefile_path ) ) != NULL ) 
			break ;
		if ( ( fp = pathfopen ( TABLEFILE, "r", JumanPath, tablefile_path ) ) != NULL ) 
			break ;
		error ( (int*(*)(void))OpenError, "can't open", TABLEFILE, ".", EOA ) ;
	}
...
void check_edrtable ( MRPH *mrph_p, CELL *x ) /* for EDRdic '94.Mar */ {
	int i;
	for ( i=0; i<TBL_NUM; i++ ) {
		if ( ( rensetu_tbl[i].hinsi == -1 ) && !strcmp ( _Atom(x), rensetu_tbl[i].goi ) ) {
			mrph_p->con_tbl = i ;
			return;
		}
	}
	error ( (int*(*)(void))OtherError, "No morpheme in EDR-table !!", EOA ) ;
}
...
void check_table ( MRPH *mrph_p ) {
	...
	for ( i=0; i<TBL_NUM; i++ ) {
		if ( ( mrph_p->hinsi == rensetu_tbl[i].hinsi ) 
			...
			&& ( ( rensetu_tbl[i].goi == NULL ) || !strcmp(mrph_p->midasi,rensetu_tbl[i].goi) ) ) {
			mrph_p->con_tbl = i ;
			return ;
		}
	}
	error ( (int*(*)(void))OtherError, "No morpheme in table !!", EOA ) ;
}
...
void connect_matrix ( FILE *fp_out ) {
	...
	getpath ( CurPath, JumanPath ) ;
	while ( 1 ) {
		if ( ( fp = pathfopen ( MATRIXFILE, "r", "", matrixfile_path ) ) != NULL )
			break ;
		if ( ( fp = pathfopen ( MATRIXFILE, "r", CurPath, matrixfile_path ) ) != NULL ) 
			break ;
		if ( ( fp = pathfopen ( MATRIXFILE, "r", JumanPath, matrixfile_path ) ) != NULL ) 
			break ;
		error ( (int*(*)(void))OpenError, "can't open", MATRIXFILE, ".", EOA ) ;
	}
	...
	for ( i=0; i<I_NUM; i++ )
		for ( j=0; j<J_NUM; j++ ) {
			if ( fscanf ( fp, "%d", &num ) == EOF )
				error ( (int*(*)(void))OtherError, "No entry in matrix !!", EOA ) ;
