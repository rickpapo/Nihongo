...
FILE *my_fopen(char *filename, char *mode)
{
     ...
     if ((fp = fopen(filename, mode)) == NULL)
	  error ( (int*(*)(void))OpenError, "can't open", filename, ".", EOA);
...
FILE *my_pathfopen(char *filename, char *mode, char *path, char *filename_path)
{
     ...
     if ((fp = pathfopen(filename, mode, path, filename_path)) == NULL)
	  error ( (int*(*)(void))OpenError, "Can't open", filename_path, EOA);
...
void *my_alloc(int n)
{
     ...
     if ((p = (void *)malloc(n)) == NULL)
	  error ( (int*(*)(void))AllocateError, "Not enough memory. Can't allocate.", EOA ) ;
...
void set_jumanrc_fileptr ( char *option_rcfile, int look_rcdefault_p, int exit_rc_notfound_p ) {
	...
	*/
	...
	char *getenv(), filename[FILENAME_MAX] ;

	if ( option_rcfile ) {
		Jumanrc_Fileptr = fopen ( option_rcfile, "r" ) ;
...
#ifdef _WIN32
		GetPrivateProfileString ( "juman", "dicfile", "C:\\PROJECTS\\Nihongo\\Juman-5.1\\dic", filename, sizeof(filename), "juman.ini" ) ;
		if ( endchar(filename) != '\\' ) 
			strcat ( filename, "\\" ) ;
			...
			if ( look_rcdefault_p ) {
				...
				if ( ( Jumanrc_Fileptr = fopen ( RC_DEFAULT ,"r" ) ) == NULL ) {
					if ( exit_rc_notfound_p ) {
						fprintf ( stderr, "not found <.jumanrc> and <RC_DEFAULT> file.\n" ) ;
						exit ( 0 ) ;
					} else {
...
#else
				fprintf ( stderr, "not found <.jumanrc> file in your home directory.\n" ) ;
				exit ( 0 ) ;
#endif
			} else {
				Jumanrc_Fileptr = NULL ;
